...
fullcommands = {
    ...
    '     test': 'activates the terminal test system.',
    '     info': 'shows the terminal info.',
    '     masterhelp': 'opens the full help window, with ALL commands.',
#     '     ': '',
...
mastercommands = {
    ...
    'run XXX': 'if found, executes the program with the name entered. In this case, it would be XXX.',
    'force run XXX': 'forces the program (in this case, XXX,) to run, does NOT depend on the mode.',
    'List of run commands': 'The below commands, when entered after (run) or (force run), will execute, depending on the mode and if it is being force ran or not.',
    '     help': 'opens the basic help menu.',
    '     fullhelp': 'opens the advanced help menu. (RE)',
    '     masterhelp': 'opens the complete help menu. (RE)',
    '     toggle_debug': 'toggles the debug mode. (RE)',
    '     giveok': 'tests the (giveok) command. (RE)',
    '     givefail': 'tests the (givefail) command. (RE)',
    '     test': 'activates the terminal`s test system.',
    '     info': 'gives the terminal`s current info.',
    '     bob': 'starts the terminal`s B.O.B. system. (RE)',
...
#     'test': 'activates the terminal test system.',
#     'info': 'shows the terminal info.',
    'NOTE': 'ALL COMMANDS are shown here. none excluded. Commands with (RE) at the end are restricted commands, and only force run and debug mode are allowed to run them.',
}
...
def masterhelp():
    for command, description in mastercommands.items():
        # .ljust(10) ensures the command takes up 10 spaces
        print(f"{command.ljust(20)} : {description}")
...
#             print(Fore.RED + f"Error: '{target}' has failed to run. Reason: 'access_denied'" + Style.RESET_ALL)
        if user_choice == 'exit':
            ...
            elif target in normal_run_commands:
                print(f"Force starting {target}...")
                time.sleep(0.5)
                # This actually calls the function mapped in the dictionary
...
#             ()
            ...
            print(f"'{user_choice}' is not a valid input. type 'run help' for valid commands.")
